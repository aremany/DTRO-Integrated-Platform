...
## 프로젝트: 대구교통공사 3호선 전력관제 장애 관리 통합 플랫폼 (Integrated Platform)
...
| 구분 | 기술 | 비고 |
| :--- | :--- | :--- |
| **Frontend** | HTML5, Vanilla JS, Bootstrap, CSS3 | |
| **Backend** | Node.js (Express), Python (Flask, FastAPI) | Microservices Architecture |
| **AI/LLM** | Ollama, Gemma 3 (E2B/E4B), Unsloth | Fine-tuned Local LLM |
| **RAG** | ChromaDB, Qdrant | Vector Search |
| **ML/DL** | PyTorch (GRU), Scikit-learn (KNN) | Time-series Prediction |
| **Visualization** | Inkscape (SVG), Chart.js | Interactive Simulator |
...
### 🎉 2026-01-11: 파인튜닝 모델 공개 (Major Update)
...
- **베이스 모델**: Qwen3-8B (Alibaba Cloud)
- **파인튜닝 기술**: Unsloth + LoRA (4-bit 양자화)

#### 개발 배경 및 성과
- **최종 성공**: 4차례의 시도 끝에 2026년 1월 11일 전력계통 전문 모델 학습 완료
- **한계 극복**: 기존 Gemma-3n 모델의 부족했던 도메인 전문성을 파인튜닝으로 해결
- **시너지 극대화**: **"파인튜닝된 지식(Brain)" + "RAG의 실시간 정보(Book)"** 결합으로 통합 플랫폼의 장애 분석 성능을 실무 전문가 수준으로 끌어올림

#### 플랫폼 업데이트 사항
- **권장 모델 변경**: 통합 플랫폼의 기본 분석 모델을 `Smoothie-Qwen3-8B-DTRO`로 변경 권장
- **코드 수정 가이드**: README.md에 모델 변경 시 `analysis_api.py` 수정 방법 추가
- **범용성 유지**: 타 도메인 적용 시에는 기존 Gemma-3n 모델도 계속 지원

---

*최종 업데이트: 2026년 1월 11일 - 파인튜닝 모델 공개*
